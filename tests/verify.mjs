...
const guardCases = [
	...
	// --- per-command flag policy: dangerous for interpreters, ordinary elsewhere ---
	["python -c blocked", "bash", { command: "python3 -c print(1)" }, true, true],
	["python -m blocked", "bash", { command: "python3 -m http.server" }, true, true],
	["node -e blocked", "bash", { command: "node -e 1+1" }, true, true],
	["node --require blocked", "bash", { command: "node --require hook.js app.js" }, true, true],
	["pytest -p blocked", "bash", { command: "pytest -p evil tests" }, true, true],
	["clustered python -ic blocked", "bash", { command: "python3 -ic script.py" }, true, true],
	["git log -p allowed", "bash", { command: "git log -p -3" }, true, false],
	["head -c allowed", "bash", { command: "head -c 200 src/a.ts" }, true, false],
	["wc -c allowed", "bash", { command: "wc -c src/a.ts" }, true, false],
	["pytest -m marker allowed", "bash", { command: "pytest -m smoke tests" }, true, false],
	["pytest -xvs allowed", "bash", { command: "pytest -xvs tests" }, true, false],
	["rg --pre still blocked", "bash", { command: "rg --pre touch pattern" }, true, true],
	["git --exec-path still blocked", "bash", { command: "git --exec-path=/tmp log" }, true, true],
	["npm view blocked (network)", "bash", { command: "npm view react" }, true, true],
	["npm outdated blocked (network)", "bash", { command: "npm outdated" }, true, true],
	["npm ls allowed", "bash", { command: "npm ls" }, true, false],
	["cargo build allowed", "bash", { command: "cargo build" }, true, false],
